...
function UpdateNews({ data }: UpdateNewsProps) {
  return (
    <div className="hidden md:flex container mx-auto my-6 ">
      ...
      {/* Ads Left UpdateNews */}
      <div className=" z-[1] min-w-[300px] min-h-[600px] absolute left-0 " >
        <AdUnit
        ...
        />
        <AdSlot id="TA_Desktop_Home_Left"/>
      </div>
            ...
            <div className="flex mb-3" key={index}>
              {item.image_url && item.image_url.length > 0 && (
                <Link href={`/news/${item.news_id}`} className='cursor-pointer'>
                  <OptimizedImage src={item.image_url} alt={item.news_title ?? 'ส่วนของข่าวอัปเดต'} width={300} height={300} className='h-full w-48 object-cover rounded-tl-md rounded-bl-md' />
                </Link>)
              }
          ...
          <div className="text-center mx-auto my-[8px]">
            <div className="z-[1] min-w-[300px] min-h-[250px]">
              <AdUnit
                ...
                Mxauto='mx-auto'
              />
              <AdSlot id="TA_Desktop_Home_Right_sticky"/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className=" z-[1] min-w-[300px] min-h-[600px] absolute right-0 " >
        <AdUnit
        ...
        />
        <AdSlot id="TA_Desktop_Home_Right"/>
      </div>
